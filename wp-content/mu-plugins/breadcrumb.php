...
Description: Add breadcrumb function | the_breadcrumb()
*/
function the_breadcrumb() {
    ...
    if ( !is_front_page() ) {
        ...
        echo '<div class="' . $breadcrums_id . '" class="' . $breadcrums_class . '">';
        ...
        if ( is_archive() && !is_tax() && !is_category() && !is_tag() ) {
            ...
            if($post_type != 'post') {
                ...
                //$post_type_archive = get_post_type_archive_link($post_type);
                if( $post_type_object->name == "etudes" ) {
                    $iIdPage = 24;
                } elseif( $post_type_object->name == "laboratoire" ) {
                    $iIdPage = 327;
                } elseif( $post_type_object->name == "equipe" ) {
                    $iIdPage = 335;
                } elseif( $post_type_object->name == "thematiques" ) {
                    $iIdPage = 20;
                } elseif( $post_type_object->name == "projets" ) {
                    $iIdPage = 26;
                }

                echo '<span class="item-cat item-custom-post-type-' . $post_type . '"><a class="bread-cat bread-custom-post-type-' . $post_type . '" href="' . get_permalink($iIdPage) . '" title="' . $post_type_object->labels->name . '">' . $post_type_object->labels->name . '</a></span>';
                echo '<span class="separator"> ' . $separator . ' </span>';
            ...
            // Search results page
            echo '<span class="item-current item-current-' . get_search_query() . '"><strong class="bread-current bread-current-' . get_search_query() . '" title="Résultats de la recherche ' . get_search_query() . '">Recherche &laquo; ' . get_search_query() . ' &raquo;</strong></span>';
